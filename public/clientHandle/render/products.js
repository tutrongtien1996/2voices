    ...
    if(results.data){
        if (results.success && results.data.length > 0) {
            results.data.forEach(item => {
                const jsonDetail = JSON.stringify(item).replace(/"/g, '&quot;');
                html += `<tr class="border-bottom" data-detail="${jsonDetail}">
                    ...
                    <td class="col_action">
                        <ul class="list-inline mt-3" data-id="${item.id}">
                            ...
                            <li class="list-inline-item download_text_details"><a id="" class="download text-dark" href="${item.url_audio}" download="voice-text-to-speech.mp3"><i class="fa-solid fa-download"></i></a></li>
